...
import { postReviews, editReview } from '../Redux/Actions';
...
const EditModal = (props) => {
    ...
    const handlePostReviews = (Token, Id, Title, Desc, Rating) => {
        if (Title != '' && Desc != '' && Rating != 0) {
            dispatch(editReview(Token, Id, Title, Desc, Rating))
            dispatch({ type: "LOADING" })
        } else dispatch({ type: "MESSAGE", message: "Invalid review" })
    }

    const [title, setTitle] = useState(props.theTitle)
    const [desc, setDesc] = useState(props.theDesc)
    const [rating, setRating] = useState(props.theRating)
                    ...
                    <View style={styles.modalView}>
                        ...
                        <View style={{ flexDirection: "row" }} >
                            <TouchableHighlight
                                ...
                                onPress={() => {
                                    handlePostReviews(token, props.reviewId, title, desc, rating)
